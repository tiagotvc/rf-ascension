...
{gold:Arma — catálogo por atributo}
Pra arma, em vez de herdar o afixo inteiro da Roxa, existem 9 variantes — cada uma grava um atributo específico na peça final. Efeito final = o valor que a Roxa doadora já tinha nesse mesmo atributo (0 se ela não tinha) + o bônus fixo abaixo. Materiais: 1x Arma Intense + 1x Arma Roxa (doadora, qualquer uma, destruída) + 20x Catalisador Superior + 1x Superior Weapon Recipe.

| Atributo | Efeito final (doadora + bônus) | Chance | Custo |
|---|---|---|---|
| Strong (PvP) | PvP Attack: doadora +5% | 50% | 20kk Dalant |
| Vampiric | Lifesteal: doadora +3% (teto 8%) | 50% | 20kk Dalant |
| Advanced Strength (HP) | Max HP: doadora +6% (teto 14%) | 50% | 20kk Dalant |
| Solid (Defesa PvP) | PvP Defense: doadora +6% (teto 16%) | 50% | 20kk Dalant |
| Anti-Sharp | Resistência a dano crítico: doadora +2% | 50% | 20kk Dalant |
| Sharp | Accuracy: doadora +5 | 50% | 20kk Dalant |
| Grand | Avoidance: doadora +5 | 50% | 20kk Dalant |
...
| 31-35 | 4 | !icon[Armor Box Lv.35](/game-data/dungeon/icons/bxgem35.png) Lv.35 x2 | x2 | Yes (x2) | — |
| 36-40 | 4 | !icon[Armor Box Lv.39](/game-data/dungeon/icons/bxgem39.png) Lv.39 x1 | x1 | Yes (x2) | Yes |
...
const STRAY_COMBOS_TITLE = "Catálogo de Combinações (Combine Ex) — todas as receitas por atributo";

// 2026-09-11 - limpeza: esse catálogo virou um tópico separado em Classes & Builds (COMBOS_SLUG) por
// engano - o conteúdo (a parte de Superior) foi incorporado em SUPERIOR_TOPIC_BODY, dentro do tópico
// que já existia em Informações do servidor. Remove o tópico órfão se ele ainda existir (idempotente -
// não faz nada depois da primeira vez que limpa).
async function removeStrayItemCombosTopic(db: Db) {
  await db.execute(
    sql`DELETE FROM forum_posts WHERE topic_id IN (SELECT id FROM forum_topics WHERE forum_slug = ${COMBOS_SLUG} AND title = ${STRAY_COMBOS_TITLE})`
  );
  await db.execute(sql`DELETE FROM forum_topics WHERE forum_slug = ${COMBOS_SLUG} AND title = ${STRAY_COMBOS_TITLE}`);
}
